...
import com.example.yygh.sta.feign.staFeignClient;
...
@Service
@Slf4j
public class staRamActuatorCore implements ramActuatorCore {
    @Autowired
    private staFeignClient staFeignClient;

    @Override
    public RamEntity saveRam() {
        String ram = staFeignClient.getRam();
        log.info(ram);
        ...
        ramEntity.setServiceName("service_sat");
        return ramEntity;
    }
}
